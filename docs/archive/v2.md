...
> Historical design note. This document records the original plan; the
> current API and internals are documented in `api.md` and
> `internals.md`.
...
| `[:io <fn>]`                        | Ask the runtime to run an arbitrary side-effecting fn (db write, email send) off-thread       |
...
- **Streaming flows** (a flow that pushes intermediate output). Achievable
  by combining runtime-interpreted `:io` work with events/publishes that
  patch the UI, but not part of the core vocabulary yet.
